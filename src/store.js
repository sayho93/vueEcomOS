...
const resourceHost = 'http://localhost:8400/shared/public/route.php?';
// const resourceHost = "/shared/public/route.php?";
...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    checkEmail(state, data){
      if(data.code === 1) state.emailCheck = true;
      else state.emailCheck = false
    },
  ...
  actions: {
    checkEmail({commit}, {email}){
      return axios.get(`${resourceHost}F=UserSVC.checkEmail&email=${email}`, {crossdomain: true})
          .then(({data}) => {
            console.log(data);
            commit("checkEmail", data)
          })

      // return axios.post(`${resourceHost}F=UserSVC.checkEmail`, {email})
      //     .then(({data}) => {
      //
      //       commit('checkEmail', data)
      //     })
